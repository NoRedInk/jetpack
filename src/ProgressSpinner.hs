...
start prompt = do
  p <-
    toTask $
    Spinner.spinner Spinner.dots1SpinnerTheme (1000 * 200) $ T.unpack prompt
  ...
  case progressSpinner of
    ...
    Just (pg, startTime) ->
      toTask $ do
        ...
        let diff =
              ((fromInteger $ floor $ diffTime * 100) / 100) :: NominalDiffTime
        let diffShow = show $ realToFrac $ toRational diff
        putStrLn $
          "Finished " ++
          T.unpack prompt ++ " after   " ++ diffShow ++ " seconds"
