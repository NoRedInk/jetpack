...
assertParsingFails = assertBool "unexpected success" . (==) [] . Require.require
...
suite =
  testGroup
    ...
    , testCase "with whitespaces" $
      assertRequire "require( \t\"jquery\"\t )" [Ast.Require Ast.Js $ "jquery"]
    , testCase "js" $
      assertRequire
        "require(\"foo.bar.js\")"
    ...
    , testCase "js" $
      assertRequire
        "require(\"foo.bar.js\"), require(\"moo.bar.js\")"
        [ Ast.Require Ast.Js $ "foo" <.> "bar.js"
        , Ast.Require Ast.Js $ "moo" <.> "bar.js"
        ]
    , testCase "coffee" $
      assertRequire
        "coffee = require \"foo.bar.js\""
        [Ast.Require Ast.Js $ "foo" <.> "bar.js"]
    , testCase "js" $
      assertRequire
        "require 'foo.bar.js' "
